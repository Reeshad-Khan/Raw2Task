...
```bash
python -m py_compile \
  deeplens/projects/raw2task/data/kitti360_seg.py \
  deeplens/projects/raw2task/models/segmentation.py \
  deeplens/projects/raw2task/train_extended.py \
  deeplens/projects/raw2task/eval_robustness.py \
  deeplens/projects/raw2task/run_review_matrix.py \
  deeplens/projects/raw2task/run_end2end_pipeline.py
```

Single full co-design run:

```bash
python -m deeplens.projects.raw2task.train_extended \
  --config deeplens/projects/raw2task/configs/kitti360_review_base.yaml
```

Reviewer matrix with same split, same metric, and multiple seeds:

```bash
python -m deeplens.projects.raw2task.run_review_matrix \
  --matrix deeplens/projects/raw2task/configs/kitti360_review_matrix.yaml
```
...
```bash
python -m deeplens.projects.raw2task.run_end2end_pipeline \
  --matrix deeplens/projects/raw2task/configs/kitti360_review_matrix.yaml \
  --skip-existing
```
...
```bash
deeplens/projects/raw2task/scripts/run_full_paper_experiments.sh \
...
```bash
deeplens/projects/raw2task/scripts/run_paper_experiments.sh \
  --matrix deeplens/projects/raw2task/configs/structured_perception_matrix.yaml
```
...
```bash
deeplens/projects/raw2task/scripts/run_paper_experiments.sh --dry-run --seeds 0
```
...
```bash
deeplens/projects/raw2task/scripts/run_paper_experiments.sh --max-parallel 1
```

Robustness table for a trained checkpoint:

```bash
python -m deeplens.projects.raw2task.eval_robustness \
