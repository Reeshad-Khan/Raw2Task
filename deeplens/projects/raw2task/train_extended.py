...
Usage:
  python -m deeplens.projects.raw2task.train_extended --config deeplens/projects/raw2task/configs/kitti360_seg_identity.yaml
"""
...
# Our modules
from deeplens.projects.raw2task.sensors.cfa import LearnableCFA, soft_cfa_demosaic, superpixel_demux
from deeplens.projects.raw2task.sensors.noise import PoissonGaussianNoiseQuant
from deeplens.projects.raw2task.sensors.trainable_optics import ConstrainedFieldPSF
from deeplens.projects.raw2task.models.cnn_small import SmallCNN
from deeplens.projects.raw2task.data.kitti360_seg import (
    KITTI360Seg,
...
from deeplens.projects.raw2task.data.seg_jsonl import SegmentationJSONLDataset
from deeplens.projects.raw2task.data.occupancy_proxy import (
...
from deeplens.projects.raw2task.models.segmentation import build_segmentation_model
...
try:
    from deeplens.projects.raw2task.utils.efficiency import (
...
def _save_sensor_preview(sensor: nn.Module, out_dir: str, stem: str) -> None:
    ...
    try:
        ...
        if cfa is not None:
            ...
            ax.set_title("Learned CFA spectral weights")
            ax.set_xlabel("RGB channel")
            ax.set_ylabel("2x2 CFA site")
            ax.set_xticks([0, 1, 2])
            ...
            ax.set_xticks(x)
            ax.set_xticklabels([f"site {i}" for i in x])
            ax.set_ylim(0, 1.0)
            ax.set_ylabel("spectral weight")
            ax.set_title("CFA spectral response by 2x2 site")
