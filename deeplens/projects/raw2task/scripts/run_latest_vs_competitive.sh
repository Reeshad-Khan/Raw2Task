...
if [[ ! -f "${OCCUPANCY_OUT_ROOT}/manifests/train.jsonl" || ! -f "${OCCUPANCY_OUT_ROOT}/manifests/val.jsonl" ]]; then
  ...
  build_cmd=("${PYTHON_BIN}" -m deeplens.projects.raw2task.occupancy.kitti360_builder
    --root "${KITTI_ROOT}"
...
CUDA_VISIBLE_DEVICES="${GPU_LATEST}" "${PYTHON_BIN}" -u -m deeplens.projects.raw2task.train_unified_multitask \
  --config "${latest_cfg}" 2>&1 | tee "runs/latest_vs_competitive/latest_${MODE}.log" &
pid_latest="$!"

CUDA_VISIBLE_DEVICES="${GPU_BASELINE}" "${PYTHON_BIN}" -u -m deeplens.projects.raw2task.train_unified_multitask \
