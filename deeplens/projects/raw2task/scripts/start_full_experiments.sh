...
systemd-run --user --unit=deeplens-complete-experiments --same-dir --collect \
  ...
  /bin/bash -lc "echo \$\$ > $RUNS_DIR/complete_experiments.pid; exec \"$PYTHON_BIN\" -u -m deeplens.projects.raw2task.run_end2end_pipeline --matrix \"$MATRIX\" --skip-existing --force-robustness > \"$LOG_PATH\" 2>&1"
